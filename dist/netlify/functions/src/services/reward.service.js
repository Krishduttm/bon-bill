...
var RewardService_1;
import { Injectable, Inject, Logger, NotFoundException, BadRequestException, InternalServerErrorException, } from "@nestjs/common";
...
let RewardService = RewardService_1 = class RewardService {
    constructor(rewardRepository) {
        ...
        this.logger = new Logger(RewardService_1.name);
    }
    async generateReward(userId) {
        try {
            ...
            throw new InternalServerErrorException(`Failed to generate reward: ${error.message}`);
        }
    }
    async findByUserId(userId) {
        try {
            ...
            throw new InternalServerErrorException(`Failed to fetch user rewards: ${error.message}`);
        }
    }
    async findAll() {
        try {
            ...
            throw new InternalServerErrorException(`Failed to fetch rewards: ${error.message}`);
        }
    }
    async redeemReward(id) {
        try {
            ...
            throw new InternalServerErrorException(`Failed to redeem reward: ${error.message}`);
        }
    }
    ...
    async getUnredeemedRewardsByUser(userId) {
        try {
            ...
            throw new InternalServerErrorException(`Failed to fetch unredeemed rewards: ${error.message}`);
        }
    }
    async getTotalRewardValueByUser(userId) {
        try {
            ...
            throw new InternalServerErrorException(`Failed to calculate total reward value: ${error.message}`);
        }
    }
};
RewardService = RewardService_1 = __decorate([
    Injectable(),
    __param(0, Inject("REWARD_REPOSITORY")),
